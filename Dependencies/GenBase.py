...
from Dependencies.ClsUDBMetrics import ClsUDB_Metrics, DesignQualityAttributes, Similarity
    ...
    def _evaluate(self, x, out, *args, **kwargs):  #
        ...
        for i, item in enumerate(x):
            ...
            if self.mode == 2:
                sim = 0.0
                for i in actor_list:
                    # m = cls_udb_metric_obj.get_metrics_of_each_class(my_path=udb_path, element=i.source)
                    if i["obj"].refactoring.find("ERROR") != -1:

                        s = Similarity(mode=int(dotenv_values().get("MODE_SIM")), metrics=i["obj"])
                        sim += s.sim()
                sim = sim / len(deepcopy(gc_obj._procedure()))
                qmood_quality_attributes = DesignQualityAttributes(udb_path=udb_path)
