    ...
    def get_metrics_of_each_class(self, my_path: str, element:list = None):
        ...
        for func in db.ents("class"):
            for i in element:
                ...
                f = open("test.txt", "a").write(i+ "  =  "+ func.name() + "\n")
                if i == func.name():
...
class Similarity:
    def __init__(self, mode: int = 1, metrics: list = None):
        """
    ...
    def tsim(self, clusters, metrics):
        """
        ...
        for item in clusters:
            for i in clusters[str(item)]:
                for k, v in i.items():
                    print("K : ", k)
                    print("V : ", v)
                    print(i, " : ", "MAX : ", clusters[str(item)][str(i)]["MAX"])
                    print(i, " : ", "MIN : ", clusters[item][i]["MIN"])
    ...
    def sim(self):
        ...
        if self.mode == 1:
            for i, item in enumerate(self.metrics):
                print(f"items in sim {i}: ", item)
            # self.tsim(clusters=self.read_class_json(), metrics=)
        elif self.mode == 2:
            pass
